...
- For example, the [[Node-Controller]] monitors the status of the nodes, and taking the actions to keep the apps running. It is done via the [[04 kube-api]].
  ...
  - If it doesn't, it removes that pod from the node, and provisions them on healthy ones if the pod is part of a [[12 ReplicaSets|ReplicaSet]].
- The next controller is a replication controller, which monitors the status of [[12 ReplicaSets|ReplicaSets]].
...
- If you have [[00 kubeadm|kubeadm]], it's deployed as a pod in the control plane node, and you can view the option in the manifests folder.
